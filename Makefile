...
VERILOG_FILES = top.v attosoc.v simpleuart.v picorv32.v

chip.bin: $(VERILOG_FILES) picorv32.pcf
	yosys -q -p "synth_ice40 -json chip.json" $(VERILOG_FILES)
#	yosys -q -p "synth_ice40 -blif chip.blif" $(VERILOG_FILES)
#	arachne-pnr -d 8k -P tq144:4k -p picorv32.pcf chip.blif -o chip.txt
	nextpnr-ice40 --freq 50 --hx8k --package tq144:4k --json chip.json --pcf picorv32.pcf --asc chip.txt --opt-timing
